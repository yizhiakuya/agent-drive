...
1. **定位**：文件名明确的直接 read_document；不明确的先 search_content（关键词）或 semantic_search（语义）找到文件。
2. **阅读**：长文档先读开头(offset=0)和结尾，再按需读中间；短文档一次读完。引用关键条款时给出原文位置。
3. **回答**：基于文件实际内容作答，明确区分"文件里写了"与"我推测的"。文件里没有的内容不要编造，如实说"文件中未提及"。
4. **对比**：逐份读取每个文件，列成对照表（相同点/不同点/缺失项）。
