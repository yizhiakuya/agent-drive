...
Java API 127.0.0.1:8000 ───── PostgreSQL/pgvector
      ...
      └── frontend/out               └── 结构化状态、索引
                 ...
                 └── Identity Service 127.0.0.1:8030
                 └── Index Service 127.0.0.1:8040
...
| `services/agent-service` | 独立聊天 session/transcript/replay/confirmation/run/event 服务 | 自有 agent schema；生产 API 通过内部 token 使用，SSE 重连可跨进程 |
