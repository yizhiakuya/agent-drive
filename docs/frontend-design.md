...
- 文件 mutation 在提交前就地展示目标路径或前五个受影响路径；单文件重命名、移动、复制和移入回收站成功后，toast 可提供一次性“撤销”，撤销失败必须再次显示真实错误。
...
- 工具步骤运行中显示阶段和实时耗时，完成/失败后保留“耗时 mm:ss”；composer 状态区显示运行中的任务计时，结束后显示本轮总耗时，不能只显示笼统的 STREAMING/READY。
- 智能摄入策略在设置页使用单个 `ui/select`：手动索引、仅自动理解新图片、自动理解全部新文件。上传成功与自动索引是两个独立状态，上传队列不能等待模型调用；活动中心显示抽取、视觉描述、向量化和失败终态。
- 语义搜索默认只展示相关度不低于 30% 的结果；没有可靠命中时显示可解释空态，用户可以显式降低最低相关度，不把低分结果伪装成准确答案。
