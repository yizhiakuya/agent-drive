...
- 聊天输入区位于视口底部但保留一小段底部呼吸空间，不能贴住视口边缘；聊天模型 Combobox 的候选层固定向上展开；列表沿用组件的可用高度上限并内部滚动，不能压缩成底部窄条或遮挡输入框。
- 首次配置页的模型字段使用可输入 Combobox；填写当前边界的 API key 后才允许获取目录，协议、地址或 key 改变时清空旧目录状态，协议或地址改变时同时清空 key 草稿。
- 模型上下文以紧凑的原生 `<details>` 行显示“上下文注入 · 来源”，默认折叠；展开正文使用有界滚动的等宽文本区，不与助手回复或工具卡混成同一种视觉层级。上下文窗口控件紧跟输入区顶部的推理层级，默认显示圆环与已用/总 token，点击后在就地浮层展开上限、已用、可用空间和本轮输入/输出；点击其他失焦区域或 Escape 自动收起，不单独占据底部整行。
- 助手 Markdown 使用正常块级换行而不是继承气泡的 `pre-wrap`；段落、列表和列表项保持紧凑间距，首尾不叠加空白，代码块继续保留预格式化和滚动。
...
- 工具步骤卡默认收叠但整行可点击；有结构化 `parsed` 而没有文本 `output` 时仍必须展示可展开的 pretty JSON，确实无返回体时显示明确的空结果状态，不能让点击看起来没有反应。
- 权限模式紧跟推理等级，使用紧凑图标+文本控件；展开后以“请求批准 / 帮我批准 / 完全访问”三行单选菜单展示批准范围和简短说明，当前项使用 check 图标。请求批准模式拦截非读取操作，但绿色内部前端导航和会话计划不拦截；帮我批准模式只拦截 red，完全访问按用户授权直接执行已登记 operation。
- Agent Activity 默认展示业务动作标题和结果状态，技术工具名、operation、参数与完整 parsed 结果折叠在详情中。常见文件动作使用“浏览文件 / 搜索文件 / 统计文件 / 读取文件内容 / 更新索引”等用户语言，不能让 `backend_api` 成为主要视觉层级。
...
- 当用户滚离最新消息时，阅读位置提示固定在 composer 上沿中线，使用小型圆形下箭头和 tooltip，不使用右下角孤立的文字胶囊；点击后平滑回到最新消息。
- 切换会话或进入已有会话时默认显示最新消息；Agent 运行期间新正文、reasoning、工具步骤和计划更新自动跟随底部，避免用户反复手动下滑。
- `@` 文件选择器必须区分“进入文件夹”和“引用整个文件夹”：文件夹主动作进入，尾部提供明确的引用动作，浏览态保留返回和当前目录引用；输入框支持从剪贴板直接粘贴图片，图片以本轮 Base64 内联内容保存在 composer 中，不进入 `聊天附件`，仅在模型支持图片输入时允许发送。
- 桌面右侧文件栏的文件/文件夹项可拖入聊天 composer 附件区；投放区用边框和背景高亮反馈，成功后复用“已附加文件”展示并按 owner-relative 路径发送，不重复上传文件或绕过文件上下文校验。
- 对话工作区的会话列表和桌面文件栏支持收缩/展开与拖拽拉伸：收缩后保留 48px 图标 rail；会话栏宽度限制为 220–360px，文件栏为 260–460px；分隔轨道必须支持鼠标拖动、方向键、Home/End 和 Enter/Space 收缩。完整会话栏只在 `xl` 宽桌面显示，较窄视口改用抽屉，不让侧栏挤压聊天 composer；布局状态使用 `agent-drive-workspace-layout-v1` 持久化。
- 工作区 header 必须保持高于右侧文件栏的 stacking 层级；操作活动、导航等顶部浮层不能被文件栏覆盖。
