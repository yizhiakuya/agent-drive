...
| 模块 | 当前能力 | 依赖或边界 |
|------|----------|------------|
| 对话 | 会话、流式正文、上下文注入、reasoning、工具步骤、权限模式、确认、停止、后台不断流切换、失败会话恢复、标题摘要、本轮模型选择 | 需要已配置且可用的对话 Provider；请求批准模式拦截非读取调用，帮我批准模式只拦截 red，完全访问按用户授权直接执行已登记 operation；模型选择只覆盖当前请求 |
