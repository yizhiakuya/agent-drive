...
文件写入或变化后，API 只负责持久化和入队；Java Worker 异步完成普通文档 Tika 抽取和 Jina 文本向量，图片先生成固定结构视觉描述，再由 Jina 生成视觉向量。两类向量按 `text`/`vision` 分开记录，索引状态与文件 revision 绑定，旧版本结果不能参与检索；图片不使用 OCR。
...
5. 文件发布后由 outbox 触发索引任务，App 可查看同步进度、目标目录、权限/通知状态、扫描/上传/去重/跳过/失败/可重试统计和设备活跃状态；失败项可从诊断入口再次触发同步。

## 4. 功能地图

| 模块 | 当前能力 | 依赖或边界 |
|------|----------|------------|
| 对话 | 会话、流式正文、上下文注入、reasoning、工具步骤、权限模式、确认、停止、后台不断流切换、刷新重连、失败会话恢复、标题摘要、本轮模型选择 | 需要已配置且可用的对话 Provider；请求批准模式拦截非读取调用，帮我批准模式只拦截 red，完全访问按用户授权直接执行已登记 operation；普通 SSE 运行由 session relay 保持，跨进程重启继续执行通过 `chat.run` 任务业务 API；模型选择只覆盖当前请求 |
...
| 内容索引 | 索引状态、单文件全文写入、视觉描述写入、Jina 文本/视觉向量、向量清空、失效索引清理、revision 校验 | `/api/v1/index` 提供 owner-scoped 直接业务 CRUD；任务模块可按调用方选择负责批量或持久化执行；embedding/vision 为可选配置，图片不走 OCR |
| 任务中心 | 顶层任务统计、筛选、阶段/文件/批次进度、执行输入/结果/失败详情、取消/重试、单条删除和终态批量清理、索引重建、失效索引清理、向量清空 | PostgreSQL 是唯一任务状态源；运行中详情通过任务事件持续刷新；活动任务和活动子任务不会被清理；失效索引清理不等于清空向量 |
| 自动化 | 计划列表、启停、立即执行、下次/上次运行、失败原因、任务历史入口、每日报告和用户偏好 | 执行结果依赖 Worker 与对应 handler；执行仍落到持久任务队列 |
| 设置 | LLM、embedding、vision、模型探测、二维码配对、设备列表、统一系统状态、退出登录 | 已存 API key 默认显示掩码，可点击眼睛临时回显；服务端加密存储；状态中心汇总 DB/Worker/Provider/索引/相册同步/备份/存储/设备，并提供对应入口 |
...
- 读操作优先保持即时反馈；文件、索引、视觉、自动化和任务能力由各自业务 API 定义行为与状态。Agent 只通过登记接口按用户目标自行编排，不受任务模块之外的“必须后台”策略约束；接口返回 queued/running/terminal 时按真实状态向用户说明。
...
- 自动化计划的启停、立即执行和删除都显示明确反馈；“立即运行”只入队，不在浏览器请求内执行重任务。
...
2026-08-22 的门禁已确认：前端 29 个测试文件、215 个断言通过，lint、静态构建和 `verify:build` 通过；Android `lintDebug`、49 个 JVM 单测和 release APK（versionCode 31 / versionName 1.0.31）通过；后端 Maven 237 个测试在本机 pgvector/PostgreSQL profile 下通过。浏览器真实生产 smoke 已覆盖桌面/375px 移动端会话抽屉、会话切换默认到底、运行中流式自动跟随底部、文件夹进入与显式引用、文件预览、收藏/最近访问、语义相关度阈值、任务/设置/自动化状态和 `@文件/文件夹` 候选；生产 API/Worker/readiness、数据库迁移和 APK 下载均已复验。上下文使用量改为推理等级后的真实 token 圆环控件，持久化到会话并支持重新进入恢复，旧会话无记录时显示估算值；支持点击外部或 Escape 自动收起，权限控件支持“请求批准 / 帮我批准 / 完全访问”三档策略，阅读位置按钮改为 composer 上沿的圆形下箭头，剪贴板图片以 Base64 内联内容发送且不进入聊天附件目录。
