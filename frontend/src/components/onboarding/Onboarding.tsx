...
function OnboardingField({ id, label, value, onChange, placeholder, type = "text" }: FieldProps) {
  return (
    <div className="mb-4">
...
export default function Onboarding() {
  ...
  const handleBaseUrlChange = useCallback((value: string) => {
    if (value !== baseUrl) setApiKey("");
    setBaseUrl(value);
    invalidateModelLookup();
  }, [baseUrl, invalidateModelLookup]);
    ...
    <div className="flex min-h-screen items-center justify-center bg-bg p-5">
      <div className="w-full max-w-[640px] border border-border bg-panel p-6 sm:p-8">
        ...
        <OnboardingField
          ...
          placeholder={protocolOf(type)?.defaultBaseUrl || "https://..."}
        />
