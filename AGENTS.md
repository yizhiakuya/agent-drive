...
- **通用后端 Agent 工具**：生产 Container 只注册 `backend_api`（另有 plan/skills 辅助工具）；先 `action=discover`，再用返回的 `METHOD /api/v1/path` operation 调 `action=call`；尚无 HTTP 路由的既有能力以同一 envelope 的 `INTERNAL name` 兼容 operation 提供，不暴露独立旧工具。HTTP 目录排除 auth/chat/health 和外部 URL；模型不能提供 Cookie、Bearer、Authorization、任意请求头、Python 入口或未登记 operation。聊天只把当前 Request 的凭据放进进程内 ASGI client，Worker 使用 Container 随机的内部 gateway token。
- **通用 API 风险**：GET 和只读探测自动执行；实际写操作按 operation 动态为 yellow/red，red 必须经过签名确认和确定性重放。FastAPI 继续负责路径、查询、JSON/form/multipart Schema 校验；响应统一脱敏，二进制只返回元数据，discovery 结果需保持在 Agent 工具输出预算内。
- **向量配置 API**：`backend_api` 发现并调用 `PUT /config/embeddings`，语义与设置页一致——provider 仅 jina；api_key 留空仅当 provider/base_url/model 与已存配置一致时才沿用已存 key（改配置必须重填）；保存后测试连接，成功则 refresh_embedder + 入队 index.rebuild。状态查询必须报告 embeddings 状态（configured/provider/model/api_key_masked），勿让模型凭空声称已配置。
...
- **sanitize 分块边界**：chunk 恰好以 `<`/`<|`/`</` 结尾时 `_safe_cut` 必须显式扣留（_TAG_START lookahead 含 [/|]? 与 \\Z），否则后续 `tool_calls>` 无法成标签、标记原文泄漏（生产实测：会话正文与摘要出现 `<tool_calls>` 原文）。`summarize_session`/`_generate_title` 的 LLM 文本同样必须过 `sanitize_tool_markup`。改清洗规则必跑 tests/unit/test_sanitize.py（含任意分块穷举用例）
- **工具输出展示截断**：`_execute_tool` 的 entry 用 output[:500] 仅作兜底，parsed 用完整输出解析（勿回退 output[:500] 截断解析——长 JSON 必然断裂）；前端对象型 parsed 渲染 pretty JSON；工具参数/确认卡展示走 `maskSecretsJson`（键名含 key/token/secret 掩码），勿裸 JSON.stringify
...
- **chat SSE 解析**：chatStream 必须处理 LF/CRLF/CR、换行跨 chunk、UTF-8 码点跨 chunk、多行 data 和无终止空行的尾事件；401 与普通 API/上传共用 `EV.unauthorized`，错误保留后端 detail。**线上契约：每个 SSE data 必须是 JSON 对象**（前端解析器拒绝裸字符串）——text 事件形状 `{"text": str}`，reasoning 事件形状 `{"text": str}`，后端 chat.py 负责把 run_stream 的裸字符串 payload 包成对象（集成测试钉死契约，勿改回裸字符串）
- **对话思考等级/过程**：输入区的思考等级由 `auto/low/medium/high` 组成，默认 `auto` 以保持普通模型兼容；显式等级通过 `ChatRequest.thinking_level` 传到 Provider。Provider 输出的 reasoning 必须走独立 SSE 事件并存入 assistant 消息的 `reasoning` 字段，ChatPanel 用原生 `details` 默认收叠、允许用户展开；reasoning 不进入下一轮 history。
- **chat 流式节流**：ChatPanel 每 80ms 批量刷一帧（streamTimerRef），流结束冲刷最后一帧；勿改回逐 token setState
- **模型正文 DSML/XML 防护**：DSH 训练的模型有时在正文里模拟工具调用（<|DSML|tool_calls>…），后端不识别会原样进气泡。loop.py 的 _chat_path/_final_reply 用 agent/sanitize.py（ToolMarkupStripper 流式清洗 + sanitize_tool_markup 一次性）剔除整行/内联标记，疑似标记行流式期间扣留、flush 放行；改清洗规则必跑 tests/unit/test_sanitize.py。prompt.py 已明确「严禁在回复文本中模拟工具调用格式（DSML/XML）」。QA mock（scripts/qa-settings/mock_models.py）必须支持 stream=true 的 SSE 分片且回复固定「你好，我是QAMock助手。」（chat-stream-reply 检查依赖），勿改回非流式 pong
