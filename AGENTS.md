...
- **向量配置 Agent 入口**：`configure_embeddings` 工具（red 级）是向量服务唯一对话配置入口，语义与设置页 `PUT /config/embeddings` 一致——provider 仅 jina；api_key 留空仅当 provider/base_url/model 与已存配置一致时才沿用已存 key（改配置必须重填）；保存后测试连接，成功则 refresh_embedder + 入队 index.rebuild。`get_system_status` 必须报告 embeddings 状态（configured/provider/model/api_key_masked），勿删——生产会话曾因工具清单缺入口导致模型幻觉"已配置完成"（实际 embeddings 为 null）
