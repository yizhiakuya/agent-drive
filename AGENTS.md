...
- **API 与服务运行模式**：`backend/` 是 Java 21 API 主目录；生产由 `agent-drive-java.service` 运行 `--app.mode=api`，并由独立 `agent-drive-content.service` 承载已接入的视觉内容理解；`agent-drive-file.service` 和 `agent-drive-index.service` 已部署，Index DB 已完成 23/22 条初始快照迁移，但在 owner/索引数据重新校验完成前不接管主 API；Identity Service 仅在显式准备数据库后部署。任务/Worker unit 已移除，禁止运行时调用 Python API；Python source/unit 已删除，legacy data/system 只在显式 `migrate` profile 下作为受控迁移/人工恢复输入。
